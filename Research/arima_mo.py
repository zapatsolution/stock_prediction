...
from pylab import rcParams
rcParams['figure.figsize'] = 10, 6
df_d = min_max.fit_transform(data)
df_log = df_d[:,[3,4]]
...
# load model
fitted = ARIMAResults.load('model.pkl')

# Forecast
fc, se, conf = fitted.forecast(1500, alpha=0.05)  # 95% confidence
fc_org = min_max.inverse_transform(fc)
fc_series = pd.Series(fc, index=np.arange(test_data[0].index,len(fc)))
